classdef JsonInfoReader
    ...
    methods (Static)
        function jsonStruct = getJson()
            %%%% get the json %%%%
            % In beta folder (ONLY!) we don't want to read from local json
            % This setting is for setup 4. In other setups we need
            %   1. Maybe change folder name
            %   2. Copy relevant parts (alnost all) from local json into
            %   the json file here.
            jsonTxt = fileread('C:\\Users\\OWNER\\Google Drive\\NV Lab\\Control code\\beta\\DevOnly\\setupInfo.json');
            % jsonTxt = fileread('C:\\lab\\setupInfo.json');    % Uncomment this in prod
